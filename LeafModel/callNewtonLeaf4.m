...
while Terror > Tmaxerror && (Tempiteration <= TempMaxIteration)
    ...
    while (Aerror > Maxerror || Cierror > Maxerror) && (CO2iteration <= CO2MaxIteration)
        ...
        ComputeCO2TransportLimitationsAll % This contains the photosynthesis module and the portion of boundary layer conductance that CO2 depdendent
...
%% Post processing
% Calculating Leakage
LeafMassFlux.L_bs = Photosynthesis.gbs*(LeafState.cbs-LeafState.ci); % CO2 leakage rate from BS to M [u mol m-2 sec-1]
